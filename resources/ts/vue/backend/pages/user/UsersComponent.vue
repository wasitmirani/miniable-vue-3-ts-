                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row mb-2">
                            ...
                            <div class="col-md-6">
                                <search-input label="Search By Name,Email,Username"
                                    :apiurl="'/user?page=' +this.page_num" v-on:query="isQuery($event)"
                                    v-on:loading="loadingStart($event)" v-on:reload="getUsers()"
                                    v-on:filterData="filterData($event)"></search-input>
                            </div>
                        </div>
                        <!-- end row -->
                        <loader-box v-if="loading"></loader-box>
                        <user-table :users="users" v-else :query="query" :getUsers="getUsers"
                            v-on:editItem="editItem($event)" v-on:deleteItem="deleteItem($event)"></user-table>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <user-form :roles="roles" :editmode="editmode" :editForm="user"
                            v-on:created="closeModal($event)" v-on:updated="closeModal($event)"></user-form>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
